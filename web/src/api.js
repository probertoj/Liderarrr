...
export const api = {
  ...
  preferCopy: (id) => req(`/albums/${id}/prefer`, { method: 'POST' }),
  preferCopyAuto: (id) => req(`/albums/${id}/prefer`, { method: 'POST', body: { clear: true } }),
  ...
  combineAlbums: (ids) => req('/albums/combine', { method: 'POST', body: { ids } }),
  uncombineAlbum: (id) => req(`/albums/${id}/uncombine`, { method: 'POST' }),
  combineCandidates: (id) => req(`/albums/${id}/combine-candidates`),
  boxInfo: (id) => req(`/albums/${id}/box`),
  identifyBox: (id) => req(`/albums/${id}/box/identify`, { method: 'POST' }),
  ...
  importSubfolders: (dir) => req('/imports/subfolders?dir=' + encodeURIComponent(dir)),
  importIgnore: (sourceDir) => req('/imports/ignore', { method: 'POST', body: { sourceDir } }),
  importUnignore: (sourceDir) => req('/imports/unignore', { method: 'POST', body: { sourceDir } }),
  ...
  // biblioteca de Spotify del usuario (OAuth) + brecha disco/streaming
  spotifyUserStatus: () => req('/spotify/user/status'),
  spotifyAuthUrl: () => req('/spotify/user/authurl'),
  spotifyConnect: (code) => req('/spotify/user/connect', { method: 'POST', body: { code } }),
  spotifyDisconnect: () => req('/spotify/user/disconnect', { method: 'POST' }),
  ...
  gap: (since) => req(`/listening/gap${since ? `?since=${since}` : ''}`),
